        ...
        <form action="{{ route('laporan-akhir.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group mb-3">
                ...
                <textarea name="deskripsi_repository" id="deskripsi_repository" cols="30" rows="3"
                    ...
                    maxlength="1000"
                    placeholder="Contoh: Laporan ini membahas tentang implementasi sistem informasi...">{{ old('deskripsi_repository') }}</textarea>
            ...
            <div class="form-group mb-3">
                ...
                <input type="file" name="file_path" id="file_path"
                    class="form-control @error('file_path') is-invalid @enderror" required autofocus>
                @error('file_path')
                    <small class="text-danger">
                        {{ $message }}
                    </small>
                @enderror
                <div class="form-text text-muted">
                    Format: PDF, DOC, DOCX. Maksimal 2MB. <strong>Wajib diisi.</strong>
                </div>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select name="kategori_repository" id="kategori_repository"
                    ...
                    <option value="Teknologi Informasi" {{ old('kategori_repository') == 'Teknologi Informasi' ? 'selected' : '' }}>Teknologi Informasi</option>
                    <option value="Sistem & Infrastruktur" {{ old('kategori_repository') == 'Sistem & Infrastruktur' ? 'selected' : '' }}>Sistem & Infrastruktur</option>
                    <option value="Keamanan & Privasi" {{ old('kategori_repository') == 'Keamanan & Privasi' ? 'selected' : '' }}>Keamanan & Privasi</option>
                    <option value="Inovasi & Riset" {{ old('kategori_repository') == 'Inovasi & Riset' ? 'selected' : '' }}>Inovasi & Riset</option>
                    <option value="Lainnya" {{ old('kategori_repository') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
            ...
            <div class="d-flex justify-content-end gap-2">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="ti ti-device-floppy me-1"></i> Simpan Laporan Akhir
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    // Update counter on page load if there's old input
